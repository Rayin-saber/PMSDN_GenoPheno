...
We demonstrated the feasibility of using registry data based on patient-reported outcome questionnaires to conduct a multi-phenotype-genotype association study.
We further replicated findings of previous prospective association studies.
We also confirmed the discovery of novel findings in renal malformations and gross motor development.

This is the largest study of its kind in PMS and the first using the Phelan-McDermid Syndrome International Registry.
Access to the PMSIR allowed us to recruit the largest sample size (`r article$nb_pat$included` patients) for a genotype-phenotype association analysis for PMS (Soorya et al.(2013), 32 patients,[@soorya_prospective_2013] Sarasua et al.(2014), 70 patients,[@sarasua_22q13.2q13.32_2014] Sarasua et al.(2011), 71 patients,[@sarasua_association_2011] Sarasua et al.(2014), 201 patients [@sarasua_clinical_2014]).
The data provided by this patient-driven network differs from that obtained from medical records.
...
At home, parents have more time and flexibility to fill the surveys; find it easier to make corrections to entered data; and are more likely to answer items more thoroughly and update surveys.
...
Some of these features were found in PMS patients with interstitial deletions not affecting *SHANK3*.[@wilson_interstitial_2008 ; @vittoria_disciglio_interstitial_2014]
Such features were also found to be present to a lesser extent and severity in PMS patients with isolated *SHANK3* mutations.[@macedoni-luksic_deletion_2013 ; @bonaglia_disruption_2001 ; @delahaye_chromosome_2009]
...
We found that patients with greater deletion sizes were less likely to present common differential diagnoses (or misdiagnoses) of ADHD and ASD, as well as symptoms of both these disorders.
As previously suggested,[@sarasua_association_2011 ; @sarasua_22q13.2q13.32_2014] this phenomenon might be due to the depth of the disabilities due to the greater deletion size "masking" autostic symptoms.
...
Also associated with greater deletion sizes, were abnormal IRM and CAT scans, which are well-documented in PMS patients [@soorya_prospective_2013] as with reports of abnormal ventricles and decreased grey matter.
...
An alternative explanation is that these phenotypes are part of the core 22q13 deletion syndrome caused by the absence of *SHANK3*, and are present in nearly all of the patients in the sample.
This holds for intellectual disability, teeth grinding, joint laxity, or high pain tolerance level.

On *caveat* is that, when used as a predictor for binomial outcomes——such as the occurrence of a clearly defined somatic phenotype——deletions can only act as a proxy for the contribution of chromosomal regions of interest and hint at potential locations for breakpoints.
...
To do this, however, we need a specific model developments as gene statuses inferred from this data would generate strong collinearity between contiguous genes affected in the same deletion ranges.

We also note that the majority of patients diagnosed with PMS as caused by 22q13 deletions, are tested solely for genetic defects on chromosome 22, specifically using array CGH or FISH techniques and providing ranges of deletions.
This creates a bias in the data, ignoring possible additional chromosomal aberrations for which no testing has been done.
Therefore, in this analysis, we had to assume that other potential defects were randomly distributed and did not affect outcome.
