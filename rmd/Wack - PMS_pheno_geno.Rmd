...
csl: humangenet.csl
author: Maxime Wack, Claire Hassen-Khodja, Michael T. McDuffie, Geraldine Bliss, Liz Horn, Andria Cornell, Cartik Kothari, Brittany McLarney, Rebecca C. Davis, Kristen Hanson, Megan O'Boyle, Isaac Kohane, Catalina Betancur, Paul Avillach
abstract: |
  # **Abstract**
...
phenotypes <- function(text, quotes = "")
{
  paste0(quotes,
         paste0(text[-length(text)],
                collapse = paste0(",", quotes, " ", quotes)),
         ",", quotes, " and ", quotes,
         text[length(text)],
         quotes)
}
...
result_for_group <- function(group, quotes = "")
{
  article$results_ranges %>%
    filter(Group == group,
           p.adj < .05) %>%
  ...
  str_c(phenotypes(res$text, quotes),
        " (*p values* from ",
...
Approximately 2,487 cases worldwide (917 in the US) have been diagnosed and registered with the Phelan-McDermid Syndrome Foundation (PMSF).
No treatment are yet available, although one drug candidate, IGF1 (Insulin-like Growth Factor 1), has been used successfully in a phase 2 randomized controlled trial.[@kolevzon_pilot_2014]

Clinically, PMS patients present with autistic features and autism spectrum disorder (ASD) or autistic-like behavior at a rate ranging from 44% to 94.[@kolevzon_phelan-mcdermid_2014 ; @costales_phelanmcdermid_2015]
An estimated 84% of cases meet the criteria for autism spectrum disorder (ASD), based on the gold standard instruments of diagnosis (the Autism Diagnostic Observation Schedule and the Autism Diagnostic Interview, Revised).[@soorya_prospective_2013]
...
The disease is diagnosed by genetic testing, which identifies either a heterozygous deletion at chromosome 22q13.33 (with involvement of at least part of *SHANK3*) or a heterozygous pathogenic variant in *SHANK3*.
...
It is thought to be play a critical role in the assembly and plasticity of the postsynaptic density in brain synapses.[@Yi_heterozygous_2016]

Haploinsufficiency (deletion or loss of function of a single copy) of *SHANK3* has been shown to be sufficient to cause the disease,[@wilson_molecular_2003 ; @durand_mutations_2007] in both patient cases and murine models with *de novo* point mutations or interstitial deletions that exclusively disrupt *SHANK3*.
PMS patients with *SHANK3* sequence variants or single-gene deletions show global developmental delay, major speech delay or severely impaired speech, moderate to profound ID, autistic behavior, hypotonia, and minor dysmorphic features (dolichocephaly, epicanthic folds).[@durand_mutations_2007 ; @moessner_contribution_2007 ; @bozdagi_haploinsufficiency_2010 ; @macedoni-luksic_deletion_2013 ; @bonaglia_disruption_2001 ; @delahaye_chromosome_2009]

PMS patients exhibit heterogeneous chromosomal abnormalities in the 22q13.33 region, including terminal deletions, interstitial deletions, sequence variants, translocations, inversions, mosaicism, and ring chromosome.
...
More intriguing, the non-classical cases present interstitial deletions as small as 0.72Mb upstream of *SHANK3*.
...
The only international registry for PMS, the registry today includes 1,353 of the 2,487 patients registered with the PMSF.
Its phenotypic and genetic information is annotated by the patient families.
...
The scanned paper reports of genetic test results are uploaded by families onto the platform, then manually curated by a team of trained genetic counselors and clinicians, who fill structured fields documenting genetic abnormalities.
...
The project was approved by Harvard Medical School Institutional Review Board (HMS IRB14-2161). The PMSIR is receiving continuing review approval by the Advarra (formerly Chesapeake) IRB as of Jan 21st 2020 (CR00182785).
...
To analyze phenotypes, we selected `r article$nb_pat$included` patients who both carried terminal deletions of chromosome 22 or point mutations within *SHANK3* and had entered phenotypic information.
...
The detail of all the genetic defects found in those patients is presented in table 2.
...
Patients with the largest deletions also showed swelling of the extremities, particularly the lower limbs (`r result_for_group("Lymphatic-related conditions")`).
...
These included `r result_for_group("Gross motor milestones", quotes = '"')` (Figure 3).
...
These symptoms included `r result_for_group("Feeding difficulties")`, as well as the need for naso-gastric tubes (`r pvalue("Nasogastric tube")`) and difficulties are not explained by feeding disorders (e.g. PICA (`r pvalue("Pica")`, OR = `r or("Pica")`).
...
On the neurological side, abnormal brain MRI (`r pvalue("Abnormal brain MRI result")`)
and CT scans (`r pvalue("Abnormal cranial CT scan result")`)
in general, as well as the presence of specific features such as
irregular brain ventricles (`r pvalue("Irregular brain ventricles")`), and
decreased gray matter (`r pvalue("Decreased grey matter")`)
and myelination (`r pvalue("Decreased myelination")`) were more likely to be found in patients with larger deletions. This translated to more febrile (`r pvalue("Febrile seizures")`) and tonic-clonic (`r pvalue("Tonic-clonic seizures")`) seizures, but not in other neurological signs and symptoms that are recorded by the PMSIR.

Some behaviors such as daytime teeth grinding (`r pvalue("Daytime teeth grinding")`) were associated with larger deletions.

All phenotypes positively associated with deletion sizes, and related analyses, are presented in Table 3.
...
In terms of behavior, we found that some anxiety disorders were negatively associated with the size of deletion, namely `r result_for_group("Anxiety disorders")`.
Attention disorder symptoms were also less prevalent in patients with greater deletion sizes: specifically, `r result_for_group("Attention disorder", quotes = '"')`.
...
These included, `r result_for_group("Cognitive development", quotes = '"')`.

A range of concurrent psychiatric diagnoses (`r result_for_group("Psychiatric conditions")`) were negatively associated with deletion size
...
At the same time, verbal speech (`r result_for_group("Verbal communication")`) was heavily impaired for patients, even those with small deletions.
Some speech disorders (`r result_for_group("Speech disorders")`)
were more diagnosed in patients with shorter deletions.
However, syndromic etiologies (`r result_for_group("Communication – Non-verbal skills")`) or auditory processing disorder (`r pvalue("Auditory processing disorder")`) cannot explain the negative associations of these communication development delays with deletion sizes.
Hearing problems (apart from the installation of ear tubes (`r pvalue("Ear tubes")`) were not associated with the genetic status.

We observed delays in socio-emotional development, in all activities related to socialization (`r result_for_group("Socio-emotional development", quotes = '"')`).

Sleep disturbances (`r result_for_group("Sleep disturbances")`) are all less frequent in patients with greater deletions.

The details of all phenotypes significantly associated with negative effect sizes are presented in Table 4.
...
Such features were also found to be present to a lesser extent and severity in PMS patients with isolated *SHANK3* mutations.[@bonaglia_disruption_2001; @delahaye_chromosome_2009]
...
As previously suggested,[@sarasua_22q13.2q13.32_2014] this phenomenon might be due to the depth of the disabilities due to the greater deletion size "masking" autistic symptoms.
...
Also associated with greater deletion sizes, were abnormal MRI and CT scans, which are well-documented in PMS patients [@soorya_prospective_2013] as with reports of abnormal ventricles and decreased gray matter.
...
This would apply to phenotypes with a prevalence in our cohort similar to that in the general population and is the case for most of the phenotypes (for example, pain urinating, night terrors, enuresis, or congenital heart disease).
Phenotypes present in nearly all patients could be part of the core PMS caused by the defects of *SHANK3*.
...
However, the genetic anomalies studied here being continuous deletions, there exists a strong correlation in the deletion of contiguous genes and would necessitate specific model developments.
...
In this study, we provided a method of data curation and integration that allowed for the use of a patient-driven registry, namely the PMSIR.
...
article$table1 %>%
  as.data.frame %>%
  pander(justify = c("left", rep("center", 9), "right"),
...
```{r table2}
print("Table 2. /SHANK3/ variants in the sample")
tribble(~`Genetic defect`, ~`N (333 individuals)`, ~Mosaic,
        "Deletion", "303 (91.0%)", NA,
        "  Terminal deletion", "296 (97.7%)", 10,
        "    Simple deletion", "55 (18.6%)", 1,
        "    Unbalanced translocation", "35 (11.8%)", NA,
        "    Ring chromosome", "27 (9.1%)", 1,
        "    Other rearrangements ^a^", "6 (2.0%)", 2,
        "    Unknown ^b^", "173 (58.4%)", 6,
        "  Interstitial deletion including /SHANK3/", "7 (2.3%)", 1,
        "/SHANK3/ sequence variant", "30 (9.0%)", NA,
        "  Frameshift", "22 (73.3%)", NA,
        "  Nonsense", "4 (13.3%)", NA,
        "  Missense", "3 (10.0%)", NA,
        "  Splice", "1 (3.3%)", NA) %>%
  pander(caption = " ^a^ Other rearrangements included 2 satellited choromosomes 22, 1 pseudodicentric chromosome, 1 unbalanced inversion, and 2 uncharacterized rearrangements.\n
^b^ Patients with deletions identified with chromosomal microarray but without a karyotype to exclude the presence of a ring chromosome were classified as unknown.",
missing = "",
...
article$results_ranges %>%
  ...
  arrange(or) %>%
  select(Phenotype, p, p.adj, or, IC_OR) %>%
  mutate_each(~prettyNum(., digits = 3)) %>%
  as.data.frame %>%
  pander(split.table = Inf,
